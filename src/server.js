...
bot.onText(/\/search (.+)/, (msg, match) => {
    ...
    var options = {
        reply_markup: JSON.stringify({
            inline_keyboard: [
                [{ text: 'Add to favorites', callback_data: 'add_fav' },
                    { text: 'Show data', callback_data: 'query_to_api' }
                ],
...
bot.on('callback_query', function(msg) {
        ...
        switch (msg.data) {
            case 'query_to_api':
                try {
                    ...
                    }, function(err) {bot.answerCallbackQuery(msg.from.id, 'All currency pairs are incorrect', true);});
    ...
    }, function(err) {console.log("Failure: " + err); bot.answerCallbackQuery(msg.id, err);});
});
